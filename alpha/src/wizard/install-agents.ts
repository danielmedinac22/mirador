...
export async function installAgents(agents: AgentKey[], pkgDir: string): Promise<string[]> {
  const lines: string[] = [];
  for (const a of agents) {
    if (a === 'claude-code') lines.push(await installClaudeCode(pkgDir));
    else if (a === 'codex') lines.push(await installCodex(pkgDir));
    else if (a === 'other') lines.push(await installOther(pkgDir));
  }
  return lines;
}

async function installClaudeCode(pkgDir: string): Promise<string> {
  ...
  return `Claude Code skill at ${tildify(skillDir)}`;
}

async function installCodex(pkgDir: string): Promise<string> {
  const candidate = join(homedir(), '.codex', 'skills', 'mirador');
  try {
    await mkdir(candidate, { recursive: true });
    await cp(join(pkgDir, 'skill'), candidate, { recursive: true, force: true });
    return `Codex skill at ${tildify(candidate)} (best-effort)`;
  } catch {
    ...
    return `Codex: auto-install failed, files at ${tildify(hints)}`;
  }
}

async function installOther(pkgDir: string): Promise<string> {
  ...
  return `Manual install hints at ${tildify(hints)}`;
}
